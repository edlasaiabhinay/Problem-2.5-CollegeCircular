...
import { Search, FileText, Eye, AlertTriangle, CheckCircle } from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  const urgentCount = circulars.filter(c => c.priority === 'urgent').length;
  const avgReadRate = circulars.length > 0 ? Math.round((circulars.reduce((sum, c) => sum + (c.readBy.length / Math.max(c.totalRecipients, 1)), 0) / circulars.length) * 100) : 0;
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        ...
        <p className="text-sm text-muted-foreground mt-1">Manage and monitor all circulars</p>
      </motion.div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        <StatsCard title="Total Circulars" value={circulars.length} icon={<FileText className="w-5 h-5 text-primary-foreground" />} trend="+3 this week" />
        <StatsCard title="Read Rate" value={`${avgReadRate}%`} icon={<Eye className="w-5 h-5 text-primary-foreground" />} />
        <StatsCard title="Urgent" value={urgentCount} icon={<AlertTriangle className="w-5 h-5 text-primary-foreground" />} />
        <StatsCard title="Read" value={totalRead} icon={<CheckCircle className="w-5 h-5 text-primary-foreground" />} subtitle={`of ${circulars.length} total`} />
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {departments.map(d => (
            <button
              key={d}
              onClick={() => setFilterDept(d === 'All' ? 'all' as any : d)}
      ...
      <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
        {filtered.map((c, i) => (
          <CircularCard key={c.id || i} circular={c} index={i} />
        ))}
      </div>
